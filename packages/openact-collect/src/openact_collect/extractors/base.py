...
class Extractor(ABC):
    """Base class for activation extractors."""
    ...
    def extract(self, gen_result: "GenerationResult") -> HiddenStateData:
        """Extract activation data from a generation result.

        Args:
            gen_result: The output of ModelRunner.generate().

        Returns:
            HiddenStateData containing per-token states, mean states,
            and/or prompt-last states depending on the capture spec.
        """
        pass
