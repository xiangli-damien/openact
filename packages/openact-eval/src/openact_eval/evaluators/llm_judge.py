"""
LLM-as-judge evaluator for OpenAct-Eval.

Supports OpenAI and Anthropic backends.  API keys are resolved in order:

  1. Explicit ``api_key`` constructor argument.
  2. Environment variable:
       - OpenAI:    ``OPENAI_API_KEY``
       - Anthropic: ``ANTHROPIC_API_KEY``
  3. Framework-level config file ``~/.openact/keys.json`` (optional).

Recommended usage::

    export OPENAI_API_KEY="sk-..."
    openact-eval eval ./runs/my_run --evaluator llm

Or in Python::

    from openact_eval import LLMJudgeEvaluator
    evaluator = LLMJudgeEvaluator(backend="openai", model="gpt-4o-mini")
"""
...
_DEFAULT_JUDGE_PROMPT = (
    "You are an expert evaluator of model outputs.\n\n"
    "## Task\n"
    "Given a question, a reference (ground-truth) answer, and a model's "
    "response, determine whether the model's response is **correct**.\n\n"
...
class LLMJudgeEvaluator(Evaluator):
    ...
    api_key : str or None
        Explicit API key.  Falls back to env / config file.
    judge_prompt : str or None
        Custom prompt template with ``{prompt}``, ``{ground_truth}``,
    ...
    retry_delay : float
        Base delay (seconds) between retries (multiplied by attempt).
    parse_fn : callable or None
        Custom ``(str) -> (bool, str)`` parser for the judge output.
    """
    ...
    def __init__(
        ...
        parse_fn: Optional[Callable[[str], Tuple[bool, str]]] = None,
    ):
        ...
        if self.backend == "openai":
            try:
                ...
                raise ImportError(
                    ...
                    "pip install openact-eval[llm]"
                )
            if not api_key:
                raise ValueError(
                    "OpenAI API key not found.  Set OPENAI_API_KEY env var "
                    "or pass api_key= to the evaluator."
                )
            ...
            raise ValueError(
                f"Unknown backend '{self.backend}'.  Supported: openai, anthropic"
            )
    ...
    def evaluate_sample(self, sample: Sample) -> EvalRecord:
        ...
        try:
            ...
            return EvalRecord(
                ...
                ground_truth=ground_truth,
                error=f"Parse error: {e}.  Raw: {judge_output[:200]}",
            )
    ...
    def _default_parse(output: str) -> Tuple[bool, str]:
        """Extract (is_correct, reasoning) from judge JSON output."""
        candidates = [output]
        # also try fenced code blocks
        candidates.extend(
            re.findall(r"```(?:json)?\s*(\{.*?\})\s*```", output, re.DOTALL)
        )
